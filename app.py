...
def _meta_diag_error(err):
    if not err:
        return None
    meta = (err.get("meta") or {}) if isinstance(err, dict) else {}
    inner = meta.get("error") if isinstance(meta, dict) else None
    inner = inner if isinstance(inner, dict) else {}
    return {
        "http_status": err.get("status") if isinstance(err, dict) else None,
        "code": inner.get("code"),
        "subcode": inner.get("error_subcode"),
        "type": inner.get("type"),
        "message": (inner.get("message") or meta.get("message") or "")[:300],
    }
...
def meta_health():
    ...
    result = {
        ...
        "configured_account": bool(_account_id()),
        "me": {"ok": False},
        "adaccounts": {"ok": False, "count": 0},
        "configured_account_access": {"ok": False},
    }
    if not tokens:
        result["me"]["error"] = {"message": "No Meta access token configured"}
        return jsonify(result)

    token = tokens[0]

    me_payload, me_err = _meta_get_with_token(token, "me", {"fields": "id,name"})
    result["me"] = {
        "ok": not bool(me_err),
        "error": _meta_diag_error(me_err),
    }

    accounts_payload, accounts_err = _meta_get_with_token(
        token,
        "me/adaccounts",
        {
            "fields": "id,name,account_status,currency,timezone_name,business_name",
    ...
    account_rows = (accounts_payload or {}).get("data", []) if not accounts_err else []
    result["adaccounts"] = {
        "ok": not bool(accounts_err),
        "count": len(account_rows or []),
        "error": _meta_diag_error(accounts_err),
    }

    account_id = _account_id()
    if account_id:
        account_payload, account_err = _meta_get_with_token(
            token,
            f"act_{account_id}",
            {"fields": "id,name,account_status,currency"},
        )
        result["configured_account_access"] = {
            "ok": not bool(account_err),
            "error": _meta_diag_error(account_err),
        }
